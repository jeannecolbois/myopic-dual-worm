...
tuple<double, bool> manydualworms(double J1,
  vector<tuple<double, int*, int, int>> interactions, int* state, int statesize,
  int* d_nd, int n_nd, int* d_vd, int n_vd, int* d_wn, double beta,
  int nmaxiter, int iterworm) {
    ...
    //clean up savestate
    delete[] savestate;
    tuple<double, bool> resultingtuple = make_tuple(deltaE, updated);
...
tuple<double, int> dualworm(bool* loopclosed, int* w1, int*w2, double J1,
  ...
  double beta, int nmaxiter) {
    ...
    // pre-allocating the memory for everything that doesn't need to be created within the do...while loop
