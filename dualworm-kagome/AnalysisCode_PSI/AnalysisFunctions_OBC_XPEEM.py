...
def LoadSpinConfigs(foldername, spinconfigfile, imgfile, alpha = 0.3, xfactor = 1, yfactor = 1):
    ...
    plotSpinSites(foldername, imgfile, x/xfactor, y/yfactor, 
                  up, putimage = False, color = 'green', alpha = alpha)
    plotSpinSites(foldername, imgfile, x/xfactor, y/yfactor, 
                  down, putimage = False, color = 'orange', alpha = alpha)
    plotSpinSites(foldername, imgfile, x/xfactor, y/yfactor, 
                  undet, putimage = False, color = 'lightgrey', alpha = alpha)
    
    NonZero = sconf!=0
    red2full = [i for i in range(NonZero.shape[0])  if NonZero[i]]
    xall = np.copy(x)
    yall = np.copy(y)
    x = x[NonZero]
    y = y[NonZero]
    sconf = sconf[NonZero]
    return configlist, x/xfactor, y/yfactor,             xall/xfactor, yall/yfactor, sconf, red2full
...
def KagomeLatticeHistogram(x, y, factor = 20,bins = [0.95, 1.05, 1.68, 1.78, 1.95, 2.05, 2.59, 2.69, 2.95, 3.05, 3.41, 3.52, 3.54, 3.67, 3.95, 4.05, 4.1]):
    nspins = len(x)
    distances = []
    distances_s1s2 = {}
...
def KagomeLatticeNeighboursLists(distances_s1s2, distconds,
    ...
    #                NNList[i].append(val)
    for i in range(nn):                 
        NNdists = {key:value for (key, value) in distances_s1s2.items()
                            if distconds[i][0] < key <distconds[i][1]}
        #print(NNdists)
        NNList[i] = [value for (key,value) in NNdists.items()]
        NNList[i] = list(itertools.chain(*NNList[i]))
        
    
    if allDistances_s1s2: # if the dictionary is not empty
        print("Using allDistances")
        NNDistances_s1s2 = {key:value for (key, value) in allDistances_s1s2.items()
                            if distconds[0][0]< key < distconds[0][1]}
        NN = [value for (key,value) in NNDistances_s1s2.items()]
        NN = list(itertools.chain(*NN))
        
        
    else:
        print("Taking from NNList")
        NN = NNList[0]
    
    # correcting the lists with different types of neighbours at the same distance
    NNList3_0 = []
    NNList3_1 = []
    for (rs1,rs2) in NNList[2]:
        s1 = red2full[rs1]
        s2 = red2full[rs2]
        s1nn = [pair[1 - pair.index(s1)] for pair in NN if s1 in pair]
        s2nn = [pair[1 - pair.index(s2)] for pair in NN if s2 in pair]
        intersect = list(set(s1nn) & set(s2nn))

        if len(intersect) == 0:
            NNList3_1.append((rs1,rs2))
        else:
            NNList3_0.append((rs1,rs2))
